...
from lib.worker_testing import square3

def marking(num, queue):
    print("sub-main process is working now...")
    processes = []
    proc_num = num
    
    in_queue = mp.Queue() # 프로세스가 반환하는 결과물을 취합하기 위한 큐 생성
    ...
    for i in range(proc_num):
        ...
        proc = mp.Process(target=square3, args=(i, x[start_index:start_index+8], in_queue)) # 프로세스 번호, 작업물, 결과물 저장 큐를 넘겨준다
    ...
    while count != proc_num:
        if not in_queue.empty(): # 큐가 비어있지 않을 경우, 큐 안의 결과물을 반환한다
            results.append(in_queue.get())
            count += 1
